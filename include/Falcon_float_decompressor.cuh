...
// cuCompressor/include/Falcon_float_decompressor.cuh
//
#include <vector>
#include <cmath>
#include <cstdint>
...
{
public:
    void Falcon_decompress(float *d_decData, unsigned char *d_cmpBytes, size_t nbEle, size_t cmpSize, cudaStream_t stream);
    void Falcon_decompress_stream_optimized(
        float *d_decData,          // 解压输出缓冲
        unsigned char *d_cmpBytes, // 压缩输入缓冲（设备端）
        size_t nbEle,              // 原始元素数量
        size_t cmpSize,            // 压缩数据大小
        cudaStream_t stream);
    void decompress(const std::vector<unsigned char> &compressedData, std::vector<float> &output, int numDatas);
...
public:
    ...
    BitReader0(const std::vector<unsigned char> &buffer) : bufferPtr(buffer.data()), bufferSize(buffer.size()), bitPos(0), ownsBuffer(false) {}

    // 新增的指针构造函数 - 支持直接使用指针
    BitReader0(const unsigned char *buffer, size_t size) : bufferPtr(buffer), bufferSize(size), bitPos(0), ownsBuffer(false) {}
