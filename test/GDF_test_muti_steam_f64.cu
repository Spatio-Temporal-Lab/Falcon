...
std::string title = "";
#define NUM_STREAMS 4 // 使用16个CUDA Stream实现流水线
#define cudaCheckError(ans) { gpuAssert((ans), __FILE__, __LINE__); }
...
// 执行压缩流程函数
PipelineAnalysis execute_pipeline(ProcessedData &data, size_t chunkSize, bool visualize = false) {
    cudaDeviceSynchronize();
    // 创建时间线记录事件
    cudaEventCreate(&global_start_event);
    cudaEventCreate(&global_end_event);
    ...
    // cudaEventSynchronize(global_start_event);
    while (processedEle < data.nbEle || active > 0) {
        for (int s = 0; s < NUM_STREAMS; ++s) {
            switch (stage[s]) {
                ...
                case SIZE_PENDING:
                    // 查询尺寸已拷回？
                    if (cudaEventQuery(evSize[s]) == cudaSuccess) {
                        // 计算压缩后的字节大小
                        ...
                        // 更新偏移量
                        h_cmp_offset[s + 1] = h_cmp_offset[s] + compressedBytes;
    ...
    result.total_compressed_size = totalCmpSize;
    result.total_size = data.nbEle * sizeof(double)/1024/1024;
    result.end_time=totalTime;
    result.chunk_size=chunkSize;
    result.init_time = initTime;
    if (visualize) {
...
int test_multiple_blocksizes(const std::string &file_path, const std::vector<size_t> &block_sizes_kb) {
    ...
    printf("GPU预热中...\n");
    size_t warmup_chunk = 1024*16; // 小块预热
    ...
    for (size_t block_size_kb: block_sizes_kb) {
        ...
        const int num_runs = 3; // 运行3次
        for (int run = 0; run < num_runs; run++) {
            PipelineAnalysis result = execute_pipeline(data, chunkSize, false);
            times.push_back(result.end_time);
            times.push_back(result.init_time);
            results.push_back(result);
        ...
        // 计算平均时间（可以去掉最大最小值）
        float avg_time = (times[1]+times[3]+times[5]+times[0]+times[2]+times[4])/3; // 计算平均值
        
        printf("块大小: %zu KB, 时间: %.2f ms (runs: %.1f/%.1f/%.1f)(init:  %.1f/%.1f/%.1f)\n", 
                block_size_kb, avg_time, times[0], times[2], times[4],times[1], times[3], times[5]);
...
int test_multiple_blocksizes_generated(size_t data_size_mb, const std::vector<size_t> &block_sizes_kb,
    ...
    for (size_t block_size_kb: block_sizes_kb) {
        ...
        // 执行压缩并获取结果
        PipelineAnalysis result = execute_pipeline(data, chunkSize, true);
        results.push_back(result);
...
int test(const std::string &file_path = "", size_t data_size_mb = 0, int pattern_type = 0) {
    if (!file_path.empty()) {
        ...
        ProcessedData data = prepare_data(file_path);
        size_t poolSize = setup_gpu_memory_pool(data.nbEle, chunkSize);
        PipelineAnalysis verification = execute_pipeline(data, chunkSize,true);
        cleanup_data(data);
        return 0;
    } else if (data_size_mb > 0) {
        size_t chunkSize;
