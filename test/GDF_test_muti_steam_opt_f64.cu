...
CompressionInfo test_compression(ProcessedData data, size_t chunkSize)
{
    ...
    CompressionResult compResult = execute_pipeline(data, chunkSize);
    cudaDeviceSynchronize(); 

    PipelineAnalysis decompAnalysis = execute_decompression_pipeline(compResult, data);
    cudaDeviceSynchronize(); 
...
int test_multiple_blocksizes_generated(size_t data_size_mb, const std::vector<size_t> &block_sizes_kb,
    ...
    execute_pipeline(data, warmup_chunk);
