...
static CompressionInfo test_gorilla_compression(const std::vector<double>& data) {
    ...
    if (filtered_data.empty()) {
        std::cerr << "[Gorilla] 警告: 数据为空或全为 NaN" << std::endl;
    ...
    for (size_t batch = 0; batch < num_batches; ++batch) {
        ...
        GorillaCompressor compressor(static_cast<int>(batch_size));
        for (size_t i = start_idx; i < end_idx; ++i) {
            compressor.addValue(filtered_data[i]);
        }
        compressor.close();
        
        total_compressed_bits += compressor.get_compress_size_in_bits();
    ...
    for (size_t batch = 0; batch < num_batches; ++batch) {
        ...
        std::vector<double> decompressed = decompressor.decompress(compressed_batches[batch]);
...
int main(int argc, char** argv) {
    ...
    for (const auto& entry : fs::directory_iterator(data_dir)) {
        ...
        std::cout << "\n[Gorilla] 正在处理文件: " << entry.path().filename() << std::endl;
        std::cout << "[Gorilla] 数据量: " << data.size() << ", 分批数: " 
        ...
        for (int i = 0; i < 3; ++i) {
            auto info = test_gorilla_compression(data);
