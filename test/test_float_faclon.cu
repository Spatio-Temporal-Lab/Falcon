...
ProcessedData prepare_data(const std::string &source_path = "", size_t generate_size = 0, int pattern_type = 0) {
    ...
    // 分配固定内存
    cudaCheckError(cudaHostAlloc(&result.oriData, result.nbEle * sizeof(float), cudaHostAllocDefault));
    cudaCheckError(cudaHostAlloc((void**)&result.cmpBytes, result.nbEle * sizeof(float), cudaHostAllocDefault));
    cudaCheckError(cudaHostAlloc((void**)&result.cmpSize, sizeof(unsigned int), cudaHostAllocDefault));
    cudaCheckError(cudaHostAlloc(&result.decData, result.nbEle * sizeof(float), cudaHostAllocDefault));
    // 将数据拷贝到固定内存
...
CompressionResult execute_pipeline(ProcessedData &data, size_t chunkSize, bool visualize = false) {
    ...
    for (int i = 0; i < NUM_STREAMS; ++i) {
        cudaCheckError(cudaMalloc(&d_in[i], chunkSize * sizeof(float)));
        cudaCheckError(cudaMalloc(&d_out[i], chunkSize * sizeof(float)));
    }
    ...
    analysis.total_size = data.nbEle * sizeof(float)/1024/1024;
    analysis.comp_time = totalTime;
    analysis.comp_throughout=(data.nbEle * sizeof(float) / 1024.0 / 1024.0 / 1024.0) / (totalTime / 1000.0);
    analysis.chunk_size = chunkSize;
    analysis.kernal_comp = kernalTime;
    *data.cmpSize=totalCmpSize;
    //     CompressionInfo tmp{
    // compressionRatio,0,totalTime,(data.nbEle * sizeof(float) / 1024.0 / 1024.0 / 1024.0) / (totalTime / 1000.0),0,0,0


    //     };
    if (visualize) {
        std::cout << "===== 压缩统计 =====" << std::endl;
        std::cout << "原始大小: " << data.nbEle * sizeof(float) << " 字节 ("
                  << data.nbEle * sizeof(float) / (1024 * 1024) << " MB)" << std::endl;
        ...
        std::cout << "压缩吞吐量: " << (data.nbEle * sizeof(float) / 1024.0 / 1024.0 / 1024.0) / (totalTime / 1000.0)
                  << " GB/s" << std::endl;
...
PipelineAnalysis execute_decompression_pipeline(const CompressionResult& compResult, ProcessedData &decompData, bool visualize = false) {
    ...
    float *d_out[NUM_STREAMS];        // 解压数据输出缓冲
    ...
    for (int i = 0; i < NUM_STREAMS; ++i) {
        ...
        // 为解压数据分配缓冲
        cudaCheckError(cudaMalloc(&d_out[i], maxChunkSize * sizeof(float)));
    }
    ...
    size_t expectedSize = compData.totalElements * sizeof(float);
...
void test_chunk(int idx,ProcessedData &data) {
    ...
    // 分配设备内存
    float *d_in, *d_out_decomp;
    unsigned char *d_out_comp;
    unsigned int h_comp_size;
    
    cudaMalloc(&d_in, chunk_size * sizeof(float));
    cudaMalloc(&d_out_comp, chunk_size * sizeof(float));
    cudaMalloc(&d_out_decomp, chunk_size * sizeof(float));
    ...
    // 验证
    float *h_decomp = new float[chunk_size];
...
int test_multiple_blocksizes(const std::string &file_path, const std::vector<size_t> &block_sizes_kb) {
    ...
    for (size_t block_size_kb: block_sizes_kb) {
        ...
        size_t chunkSize = (block_size_kb * 1024) / sizeof(float);
...
void warmup()
{
        size_t nbEle = (1*1024 * 1024) / sizeof(float);
