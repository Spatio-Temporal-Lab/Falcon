...
int main()
{
    ...
    size_t nbEle = 1024*1024*4;//1024*1024*512; // 2 GB fp32 data.
    size_t cmpSize1 = 0;
    size_t cmpSize2 = 0;
    ...
    for(int i=0; i<3; i++)
    {
        cuSZp_compress_plain_f64(d_oriData, d_cmpBytes, nbEle, &cmpSize1, errorBound, stream);
    }

    // cuSZp-p testing.
    printf("=================================================\n");
    printf("=========Testing GDFC-p-f64 on REL 1E-2=========\n");
    printf("=================================================\n");
    // cuSZp compression.
    timer_GPU.StartCounter(); // set timer
    GDFC_compress_plain_f64(d_oriData, d_cmpBytes, nbEle, &cmpSize1, stream);
    ...
    // cuSZp decompression.
    timer_GPU.StartCounter(); // set timer
    //GDFC_decompress_plain_f32(d_decData, d_cmpBytes, nbEle, cmpSize1,  stream);
    ...
    // cuSZp compression.
    timer_GPU.StartCounter(); // set timer
    cuSZp_compress_outlier_f64(d_oriData, d_cmpBytes, nbEle, &cmpSize2, errorBound, stream);
    ...
    // cuSZp decompression.
    timer_GPU.StartCounter(); // set timer
    cuSZp_decompress_outlier_f64(d_decData, d_cmpBytes, nbEle, cmpSize2, errorBound, stream);
