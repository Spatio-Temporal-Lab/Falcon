...
// 原始压缩测试函数（保持不变）
CompressionInfo test_elf_compression(const std::vector<float>& original, float error_bound) {
    // 压缩测试
    uint8_t* compressed = nullptr;
    auto compress_start = std::chrono::high_resolution_clock::now();
    
    ssize_t compressed_size = elf_star_encode_32(
    ...
    ssize_t dec_size = elf_star_decode_32(
        compressed,
        compressed_size,
        decompressed.data()
    );
...
// 新增：分块压缩测试函数
CompressionInfo test_elf_compression_with_blocks(const std::vector<float>& original, float error_bound, size_t blockSize) {
    if (blockSize == 0) {
...
CompressionInfo test_elf_with_file(const std::string& file_path, float error_bound) {
    auto data = read_data_float(file_path);
    return test_elf_compression(data, error_bound);
}

// 新增：带分块的文件测试模板
CompressionInfo test_elf_with_file_blocks(const std::string& file_path, float error_bound, size_t blockSize) {
    auto data = read_data_float(file_path);
