...
CompressionInfo comp_LZ4(std::vector<float> oriData);
...
CompressionInfo comp_LZ4(std::vector<float> oriData)
{
    ...
    float compression_ratio = total_compressed / static_cast<float>(in_bytes);
    ...
    // 吞吐量计算
    float data_size_gb = in_bytes / (1024.0 * 1024.0 * 1024.0);
    float compress_throughput = data_size_gb / (total_compress_time / 1000.0);
    float decompress_throughput = data_size_gb / (total_decompress_time / 1000.0);
