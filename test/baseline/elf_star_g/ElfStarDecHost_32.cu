...
// 安全的CUDA设备状态检查（解压版本）
bool checkCudaDeviceStateForDecode32() {
...
// 修复的块解析函数（保持不变）
int elf_star_parse_blocks_fixed32(const uint8_t *compressed_data, ssize_t compressed_len,
...
ssize_t elf_star_decode_with_timing(const uint8_t *all_in_data,
    ...
    // 检查CUDA设备状态
    if (!checkCudaDeviceStateForDecode32()) {
    ...
    const int64_t total_out_bytes = total_out_elements * sizeof(float);
    ...
    float* d_out_data = cuda_mem.allocate<float>(total_out_elements);
    if (!d_out_data) {
        // printf("解压分配输出缓冲区失败\n");
        // 清理事件并返回错误
        cudaEventDestroy(decompress_start);
...
ssize_t elf_star_decode_simple_with_timing(const uint8_t *compressed_data, ssize_t compressed_len, 
    ...
    if (elf_star_parse_blocks_fixed32(compressed_data, compressed_len, &num_blocks, 
    ...
    // 分配输出缓冲区
    *out = (float*)malloc(total_elements * sizeof(float));
    if (!*out) {
        // printf("分配输出缓冲区失败\n");
        if (block_sizes) free(block_sizes);
        return -1;
