...
__device__ void calTotalCountAndNonZerosCounts_GPU(
    ...
    int non_zeros_count = 64; // 初始假设所有位置都非零
...
__device__ int buildPostOffice_GPU(
    ...
    int dp[64][32]; // dp[i][j] = 前i个位置放j个邮局的最小代价
    int pre[64][32]; // pre[i][j] = dp[i][j]对应的前一个邮局位置
...
__device__ int initRoundAndRepresentation(
    ...
    // 2. 遍历不同的z值，寻找最优解
    int max_z = min(kPositionLength2Bits[non_zeros_count], 5); // 最多5个bit
