...
namespace alp_bench { namespace patas {
...
public:
	...
	idx_t Index() const { return index; }
...
struct PatasDecompression {
	static inline EXACT_TYPE
	DecompressValue(ByteReader& byte_reader, uint8_t byte_count, uint8_t trailing_zero, EXACT_TYPE previous) {
		return (byte_reader.ReadValue<EXACT_TYPE>(byte_count, trailing_zero) << trailing_zero) ^ previous;
	}
};

}} // namespace alp_bench::patas
