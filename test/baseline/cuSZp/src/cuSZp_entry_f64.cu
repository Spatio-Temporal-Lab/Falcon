...
void GDFC_compress_plain_f64(double* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * cmp_chunk - 1) / (bsize * cmp_chunk);
    ...
    cudaMalloc((void **)&d_bitSizes, numthread  * sizeof(uint64_t));
    cudaMemset(d_bitSizes, 0, numthread  * sizeof(uint64_t));
