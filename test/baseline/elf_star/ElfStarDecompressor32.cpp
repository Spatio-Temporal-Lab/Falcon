  ...
  private:
   ...
   void next() {
     if (first) {
       ...
       if (trailingZeros < 32) {
         storedVal.i = ((read_int(31 - trailingZeros) << 1) + 1) << trailingZeros;
       } else {
         storedVal.i = 0;
       }
   ...
   void nextValue() {
     ...
     if (read_bit() == 1) {
       ...
       value.i = read_int(centerBits) << storedTrailingZeros;
...
// len = dataset_size =1000000,data1,data2,data3(data1.size = 1024)
//len = dataset_size =1000000,data1 = 1024,datan(100000000-1024*(n-1))
