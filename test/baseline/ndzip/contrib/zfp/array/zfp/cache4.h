...
public:
  ...
  BlockCache4(BlockStore4<Scalar, Codec>& store, size_t bytes = 0) :
  ...
  // perform a deep copy
  void deep_copy(const BlockCache4& c)
  {
    free();
    cache = c.cache;
    alloc();
  ...
  void get_block(size_t block_index, Scalar* p, ptrdiff_t sx, ptrdiff_t sy, ptrdiff_t sz, ptrdiff_t sw) const
  {
    ...
    if (line)
      ...
      store.decode(codec, block_index, p, sx, sy, sz, sw);
  }
  ...
  void put_block(size_t block_index, const Scalar* p, ptrdiff_t sx, ptrdiff_t sy, ptrdiff_t sz, ptrdiff_t sw) const
  {
    ...
    if (line)
      ...
      store.encode(codec, block_index, p, sx, sy, sz, sw);
  }
...
protected:
  ...
  BlockStore4<Scalar, Codec>& store; // store backed by cache
