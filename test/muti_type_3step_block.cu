...
struct PipelineAnalysis {
    float total_h2d = 0;    // 总H2D时间
    float total_comp = 0;   // 总计算时间
    float total_d2h = 0;    // 总D2H时间
    float end_time = 0;     // 结束时间
    float sequential_time = 0; // 顺序执行时间
    float comp_level = 0;   // 压缩级别
    float speedup = 0;      // 加速比
    ...
    // 加法运算符重载：累加两个PipelineAnalysis对象
    PipelineAnalysis operator+(const PipelineAnalysis& other) const {
        PipelineAnalysis result;
        
        // 累加所有浮点成员
        result.total_h2d = this->total_h2d + other.total_h2d;
        result.total_comp = this->total_comp + other.total_comp;
        result.total_d2h = this->total_d2h + other.total_d2h;
        result.end_time = this->end_time + other.end_time;
        result.sequential_time = this->sequential_time + other.sequential_time;
        result.speedup = this->speedup + other.speedup;
        result.avg_h2d = this->avg_h2d + other.avg_h2d;
        result.avg_comp = this->avg_comp + other.avg_comp;
        result.avg_d2h = this->avg_d2h + other.avg_d2h;
        result.total_size = this->total_size + other.total_size;
        
        // 对于非累加成员，取平均值或保持原值
        result.comp_level = (this->comp_level + other.comp_level) / 2.0f;
        
        // 块大小应该相同，取任意一个值
        result.chunk_size = this->chunk_size; 
        
        return result;
    }

    // 除法运算符重载：按比例缩小所有数值成员
    PipelineAnalysis operator/(float divisor) const {
        if (divisor == 0) {
            throw std::invalid_argument("Division by zero");
        }
        
        PipelineAnalysis result = *this; // 复制当前对象
        
        // 缩小所有浮点成员
        result.total_h2d /= divisor;
        result.total_comp /= divisor;
        result.total_d2h /= divisor;
        result.end_time /= divisor;
        result.sequential_time /= divisor;
        result.speedup /= divisor;
        result.avg_h2d /= divisor;
        result.avg_comp /= divisor;
        result.avg_d2h /= divisor;
        result.total_size /= divisor;
        result.comp_level /= divisor;
        
        // chunk_size 保持不变，因为它表示块大小
...
public:
    ...
    void process_chunk(
        ...
        cudaStreamSynchronize(stream);
        cudaEventSynchronize(compression_timing.comp_event);
        // 设备到主机的拷贝 (D2H) - 压缩结果
...
PipelineAnalysis execute_pipeline(ProcessedData& data, size_t chunkSize, size_t poolSize, bool visualize = false) {
    ...
    CompressionInfo a{
        data.nbEle * sizeof(double)/1024.0/1024.0,
        cmpSize/1024.0/1024.0,
        cmpSize/static_cast<double>(data.nbEle * sizeof(double)),
        0,
        tmp,
        (data.nbEle * sizeof(double) / 1024.0 / 1024.0 / 1024.0) / (tmp / 1000.0),
        0,0,0
...
int test_multiple_blocksizes(const std::string& file_path, const std::vector<size_t>& block_sizes_kb) {
    ...
    for (size_t block_size_kb : block_sizes_kb) {
        ...
        PipelineAnalysis result = execute_pipeline(data, chunkSize, poolSize, false);  
        // 执行压缩并获取结果
        for(int i=0;i<3;i++)
        {
            result=result + execute_pipeline(data, chunkSize, poolSize, false);  
        }
        result=result/4;
