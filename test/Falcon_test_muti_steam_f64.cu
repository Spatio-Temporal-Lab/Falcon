...
CompressionResult execute_pipeline(ProcessedData &data, size_t chunkSize, bool visualize = false)
{
        ...
        for (int s = 0; s < NUM_STREAMS; ++s)
        {
            ...
            case IDLE:
                if (processedEle < data.nbEle)
                {
                    ...
                    // 调用你的压缩接口（内部处理所有临时内存）
                    // FalconCompressor_1024::Falcon_compress_stream(
                    //     d_in[s],
                    //     d_out[s],
    ...
    *data.cmpSize = totalCmpSize;
    ...
    if (visualize)
    {
        ...
        // 显示每个chunk的详细信息（可选）
        if (visualize && completedChunks <= 10)
        { // 只显示前10个chunk避免输出过多
            std::cout << "\n===== Chunk详细信息 =====" << std::endl;
...
int setChunk(int nbEle)
{
    ...
    // printf("chunkSize:%d\n",chunkSize);
    printf("chunkSize:%d MB\n", chunkSize * sizeof(double) / 1024 / 1024);
    return chunkSize;
}
...
int main(int argc, char *argv[])
{
    ...
    else if (arg == "--analyze-blocks-gen" && argc >= 3)
    {
        ...
        // 创建不同大小的块序列
        // 从16mbB到512MB，以二次方增长
        std::vector<size_t> block_sizes = generate_power2_blocksizes(4 * 1024, 8 * 64 * 1024);
    ...
    else if (arg == "--analyze-blocks-dir" && argc >= 3)
    {
        ...
        // 从16mbB到512MB，以二次方增长
        // std::vector<size_t> block_sizes = generate_power2_blocksizes(4*1024, 128*1024);
        std::vector<size_t> block_sizes = {256 * 1024};
