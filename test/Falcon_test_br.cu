...
CompressionInfo test_compression(ProcessedData data, size_t chunkSize)
{
    ...
    CompressionResult compResult = ex.executeCompressionPipelineBr(data, chunkSize);
...
void warmup()
{
        ...
        CompressionResult compResult = ex.executeCompressionPipelineBr(data, warmup_chunk);
        cudaDeviceSynchronize(); 
}
