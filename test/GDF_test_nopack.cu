...
CompressionInfo test_compression(ProcessedData data, size_t chunkSize)
{
    ...
    CompressionResult compResult = ex.executeCompressionPipelineNoPack(data, chunkSize);
...
void warmup()
{
        ...
        CompressionResult compResult = ex.executeCompressionPipelineNoPack(data, warmup_chunk);
        cudaDeviceSynchronize(); 
}
