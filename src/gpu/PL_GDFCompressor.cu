...
__device__ inline long double2long(double data,int maxDecimalPlaces, int maxBeta)
{
    ...
    // }
    return (maxBeta > 15)
    ? static_cast<long>(__double_as_longlong(data))
...
void GDFCompressor::compress(double* input, unsigned char* output, size_t nbEle, size_t* cmpSize, cudaStream_t stream)
{
    size_t inputSize = nbEle;
    ...
    // 启动核函数
    compressBlockKernel<<<numBlocks, blockSize, sharedMemSize,stream>>>(
        d_input,
        inputSize,
        d_output,
        d_bitSizes,
    ...
    // 计算吞吐量
    size_t dataSizeBytes = nbEle * sizeof(double); // 原始数据量
