...
void GDFDecompressor::decompress(const std::vector<unsigned char>& compressedData, std::vector<double>& output, int numDatas) {
    ...
    std::cout << "解压缩核函数时间" << milliseconds << " ms. \n";
...
void GDFDecompressor::GDFC_decompress_op(double* d_decData, unsigned char* d_cmpBytes, 
    ...
    // 直接使用传入的 offsets，无需重复计算和D2H传输
    int* d_offsets;
    cudaMalloc(&d_offsets, numOffsets * sizeof(int));
    cudaMemcpyAsync(d_offsets, offsets, numOffsets * sizeof(int), 
                    cudaMemcpyHostToDevice, stream);

    // 优化的核函数调用
    int threadsPerBlock = 128;
    int blocksPerGrid = (numOffsets + threadsPerBlock - 1) / threadsPerBlock;

    decompressKernelOptimized<<<blocksPerGrid, threadsPerBlock, 0, stream>>>(
        d_cmpBytes, d_decData, d_offsets, numOffsets, nbEle);
