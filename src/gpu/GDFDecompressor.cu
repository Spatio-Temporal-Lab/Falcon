...
// // 高度优化的GDFC_decompress函数(流式，仅压缩无主机)
...
//     // 读取块数（这是唯一的D2H传输，只有4字节）
//     int numBlocks;
//     cudaMemcpyAsync(&numBlocks, d_numBlocks, sizeof(int), cudaMemcpyDeviceToHost, stream);
//     cudaStreamSynchronize(stream); // 只需要同步这一个小的传输
