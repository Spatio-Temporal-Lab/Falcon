...
//
    ...
    // ZigZag 解码函数
    int64_t zigzag_decode0(uint64_t n) {
        return (n >> 1) ^ -(n & 1);
    }
    ...
    void GDFDecompressor0::decompress(const std::vector<unsigned char>& compressedData, std::vector<double>& output,int numDatas) {
        ...
        while(reader.getBitPos() + 64 + 64 + 8 + 8 + 8 +64 <= dataSize) { // 确保有足够的元数据
            ...
            for(int i =0; i < numData; i++) {
                deltas[i] = zigzag_decode0(deltasZigzag[i]);
            }
