...
__global__ void GDFC_compress_kernel_spare(
    const double* input,
    unsigned char* output,
    volatile unsigned int* const __restrict__ cmpOffset, // 压缩数据偏移量数组（输出）
    volatile unsigned int* const __restrict__ locOffset, // 局部偏移量数组（输出）
...
void GDFCompressor_opt::GDFC_compress_spare(double* d_oriData, unsigned char* d_cmpBytes, unsigned int* d2h_async_totalBits_ptr, size_t nbEle, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * DATA_PER_THREAD - 1) / (bsize * DATA_PER_THREAD);
    ...
    GDFC_compress_kernel_spare<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_oriData, d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, nbEle);
...
__global__ void GDFC_compress_kernel_string(
    const double* input,
    unsigned char* output,
    volatile unsigned int* const __restrict__ cmpOffset, // 压缩数据偏移量数组（输出）
    volatile unsigned int* const __restrict__ locOffset, // 局部偏移量数组（输出）
...
)
{
    ...
    for (int i = 0; i < numDatas; i++) {
        ...
        double alpha = getDecimalPlaces_string(value, sp);// 得到小数位数
        double beta =  alpha + sp + 1;
        maxBeta = device_max(maxBeta,beta);
        maxDecimalPlaces = device_max(maxDecimalPlaces, alpha);
    }
...
//返回bits
void GDFCompressor_opt::GDFC_compress_string(double* d_oriData, unsigned char* d_cmpBytes, unsigned int* d2h_async_totalBits_ptr, size_t nbEle, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * DATA_PER_THREAD - 1) / (bsize * DATA_PER_THREAD);
    ...
    GDFC_compress_kernel_string<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_oriData, d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, nbEle);
