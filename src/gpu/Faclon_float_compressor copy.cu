...
)
{
    ...
    for (int j = 0; j < (numDatas + 31) / 32; j++)
    {                                                   // 每个线程的数量 / 一个数据批次（32）
...
// 主压缩函数
void GDFCompressor::compress(const std::vector<float> &input, std::vector<unsigned char> &output)
{
    // std::cout<<"begin1\n";
    size_t inputSize = input.size();
    if (inputSize == 0)
...
// 返回的cmpSize是BYTE
void GDFCompressor::GDFC_compress(float *d_oriData, unsigned char *d_cmpBytes, size_t nbEle, size_t *cmpSize, cudaStream_t stream)
{
...
// 返回bits
void GDFCompressor::GDFC_compress_stream(float *d_oriData, unsigned char *d_cmpBytes, unsigned int *d2h_async_totalBits_ptr, size_t nbEle, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * DATA_PER_THREAD - 1) / (bsize * DATA_PER_THREAD);
