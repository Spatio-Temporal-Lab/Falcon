...
//
#include "FalconCompressor_1024.cuh"
...
// 初始化设备内存
void FalconCompressor_1024::setupDeviceMemory(
...
// 更新释放设备内存的函数
void FalconCompressor_1024::freeDeviceMemory(
    double* d_input,
    unsigned char* d_output,
    uint64_t* d_bitSizes
) {
...
// 主压缩函数
void FalconCompressor_1024::compress(const std::vector<double>& input, std::vector<unsigned char>& output) {
    //std::cout<<"begin1\n";
    size_t inputSize = input.size();
...
//返回的cmpSize是BYTE
void FalconCompressor_1024::Falcon_compress(double* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, cudaStream_t stream)
{
...
//返回bits
void FalconCompressor_1024::Falcon_compress_stream(double* d_oriData, unsigned char* d_cmpBytes, unsigned int* d2h_async_totalBits_ptr, size_t nbEle, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * DATA_PER_THREAD - 1) / (bsize * DATA_PER_THREAD);
...
// 主压缩函数
void FalconCompressor_1024::Falcon_compress_no_pack(double* d_oriData, unsigned char* d_cmpBytes, unsigned int* d2h_async_totalBits_ptr, size_t nbEle, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * DATA_PER_THREAD - 1) / (bsize * DATA_PER_THREAD);
...
//返回bits
void FalconCompressor_1024::Falcon_compress_br(double* d_oriData, unsigned char* d_cmpBytes, unsigned int* d2h_async_totalBits_ptr, size_t nbEle, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * DATA_PER_THREAD - 1) / (bsize * DATA_PER_THREAD);
