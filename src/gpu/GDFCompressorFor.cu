...
)
{
    ...
    int maxBeta =0;
    long refenceValue = 999999999;
    double refenceDouble = 999999999.9;
    int bitCount = 0;
    ...
    for (int i = 0; i < numDatas; i++) {
        ...
        refenceDouble = device_min(refenceDouble, value);
        maxDecimalPlaces = device_max(maxDecimalPlaces, alpha);
    }
    uint64_t maxDelta = 0;
    refenceValue = double2long(refenceDouble, maxDecimalPlaces,maxBeta); // 量化第一个
    prevQuant = refenceValue;// 初始化第一个量化值
    for(int j = 0; j < (numDatas+31) / 32; j++) { // 每个线程的数量 / 一个数据批次（32）
        ...
        if(base_block_end_idx < totalSize) {
                ...
                quant_chunk_idx = j * 32 + (i % 32); //处理的每一组的第几个数据
                    ...
                    lorenQuant = currQuant - refenceValue; // 计算差分
                ...
                lorenQuant = currQuant - refenceValue;
    ...
    // 5. 前缀和计算
        ...
        unsigned long long refenceValueBits = 0;
        memcpy(&refenceValueBits, &refenceValue, sizeof(long));
        // 6.1 写入 bitSize (8 字节)
            for(int i = 0; i < 8; i++) {
            ...
            // 6.2. 写入 refenceValue (8 字节)
            for(int i = 0; i < 8; i++) {
                output[outputIdx + 8 + i] = (refenceValueBits >> (i * 8)) & 0xFF;
...
)
{
    ...
    int maxDecimalPlaces = 0;
    int maxBeta =0;
    long refenceValue = 9999999999;
    double refenceDouble = 9999999999.9;
    int bitCount = 0;
...
)
{
    ...
    // 2. FOR + zigzag（用4向量化进行实现）
    uint64_t maxDelta = 0;
    refenceValue = double2long(input[startIdx], maxDecimalPlaces,maxBeta); // 量化第一个
    prevQuant = refenceValue;// 初始化第一个量化值
    for(int j = 0; j < (numDatas+31) / 32; j++) { // 每个线程的数量 / 一个数据批次（32）
        base_block_start_idx = startIdx + j * 32;           //每一组32个数据的起始位置
        base_block_end_idx = base_block_start_idx + 32;     //每一组32个数据的结束位置
        ...
        if(base_block_end_idx < totalSize) {
                ...
                if(i == startIdx) { // 针对第一个数据
                    // refenceValue = double2long(tmp_buffer.x, maxDecimalPlaces); // 量化当前数据点
                    // prevQuant = refenceValue;
                    // printf("refenceValue:%d\n",refenceValue);
...
)
{
    ...
    refenceValue = double2long(input[startIdx], maxDecimalPlaces,maxBeta); // 量化第一个
    // if(idx<=2)
    // printf("input:%f,fs:%ld,maxDecimalPlaces:%d,beta:%d\n",input[startIdx],refenceValue,maxDecimalPlaces,maxBeta);
    prevQuant = refenceValue;// 初始化第一个量化值
    for(int j = 0; j < (numDatas+31) / 32; j++) { // 每个线程的数量 / 一个数据批次（32）
        base_block_start_idx = startIdx + j * 32;           //每一组32个数据的起始位置
        base_block_end_idx = base_block_start_idx + 32;     //每一组32个数据的结束位置
