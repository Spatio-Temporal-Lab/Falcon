...
// 优化后的 Faclon_float_decompressor.cuh.cu
...
__global__ void decompressKernelOptimized(
    ...
    if (useDirectConversion) {
        ...
        output[blockId * 1024] = decodeFloatWithSignLast(bits);
    } else {
        output[blockId * 1024] = (float)prevValue / scale;
    }
    ...
    for (int i = 1; i < numData; i++) {
        int32_t delta = zigzag_decode(deltasZigzag[i]);
        prevValue += delta;
        
        if (useDirectConversion) {
            uint32_t bits = (uint32_t)prevValue;
            output[blockId * 1024 + i] = decodeFloatWithSignLast(bits);
        } else {
            output[blockId * 1024 + i] = (float)prevValue / scale;
        }
    }
    // if(blockId==0){
...
void GDFDecompressor::decompress(const std::vector<unsigned char>& compressedData, std::vector<float>& output, int numDatas) {
...
void GDFDecompressor::GDFC_decompress(float* d_decData, unsigned char* d_cmpBytes, size_t nbEle, size_t cmpSize, cudaStream_t stream) {
...
void GDFDecompressor::GDFC_decompress_stream_optimized(float* d_decData,
...
// 最终优化的接口函数
void GDFDecompressor::GDFC_decompress_ultra_optimized(float* d_decData,
                                                     unsigned char* d_cmpBytes,
                                                     size_t nbEle,
                                                     size_t cmpSize,
                                                     cudaStream_t stream) {
